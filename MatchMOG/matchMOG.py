...
microservices = ["detection_RIGHT", "detection_LEFT, broker", "training: [fusion, overlay]"]

encode = [2.69,3.16,2.7,28,61] #seconds
splittingstream = [11.7,  11.2, 2.1,   2.5,   3.1 ] #seconds
training = [33.23, 56.746 , 36.253 , 466.830 , 1000] #seconds
...
BW_r = [60000000 , 920000000, 800000000 , 450000000 , 328000000]#bps
...
T[0][4] = encode[4] + ((video_size[4])/BW_r[4]) + (1e-3)
...
T[1][4] = splittingstream[4] +  (60*(seg_size[index_of_segment])/BW_r[4]) + (1e-3) + (1e-3)
...
T[2][4] = training[4] + (60*(seg_size[index_of_segment])/BW_r[4]) + (1e-3) + (1e-3)
...
for i in range(len(microservices)):
	...
	sorted_dictlistMicroservices[i]=sorted(dictlistMicroservices[i].items(),key = itemgetter(1), reverse=False)
