...
splittingstream = [31,   31,  117,  112, 204] #seconds
...
fileObject = open("steps.yml", 'w').close()
fileObject = open("resources.yml",'w').close()
...
for i in range(len(microservices)):
	...
	with open(r'steps.yml', 'a') as file:
...
for j in range(len(resources)):
		...
		dicttttt = {listofkeys[0]:listofvalues}
		with open(r'resources.yml', 'a') as file:
			documents = yaml.dump(dicttttt, file)
			file.close()


resident__ = (open('steps.yml', 'r'))
resident_preferences = yaml.full_load(resident__)
resident__.close()
hospital__ = (open('resources.yml', 'r'))
hospital_preferences = yaml.full_load(hospital__)
hospital__.close()
hospital_cap = (open('capacities.yml', 'r'))
